...
public class JobEntryHadoopCopyFilesTest {
  ...
  public void setup() {
    ...
    jobEntryHadoopCopyFiles = new JobEntryHadoopCopyFiles( testName, namedClusterManager );
    testUrl = "testUrl";
  ...
  public void testNoArgConstructor() {
    jobEntryHadoopCopyFiles = new JobEntryHadoopCopyFiles();
    assertEquals( "", jobEntryHadoopCopyFiles.getName() );
  }
  ...
  public void testLoadUrlMapRNotNullNoPrefix() {
    ...
    when( namedClusterManager.processURLsubstitution( testNcName, testUrl, HadoopSpoonPlugin.MAPRFS_SCHEME, metaStore,
      jobEntryHadoopCopyFiles.getVariables() ) ).thenReturn( testNewUrl );
    String expected = HadoopSpoonPlugin.MAPRFS_SCHEME + "://" + testNewUrl;
    assertEquals( expected, jobEntryHadoopCopyFiles.loadURL( testUrl, testNcName, metaStore, mappings ) );
    verify( mappings ).put( expected, testNcName );
  }
  ...
  public void testLoadUrlNotMapR() {
    ...
    when( namedClusterManager.processURLsubstitution( testNcName, testUrl, HadoopSpoonPlugin.HDFS_SCHEME, metaStore,
      jobEntryHadoopCopyFiles.getVariables() ) ).thenReturn( testNewUrl );
    assertEquals( testNewUrl, jobEntryHadoopCopyFiles.loadURL( testUrl, testNcName, metaStore, mappings ) );
    verify( mappings ).put( testNewUrl, testNcName );
  }

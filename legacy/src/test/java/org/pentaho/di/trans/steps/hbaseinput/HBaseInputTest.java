...
public class HBaseInputTest {
  ...
  protected RowMetaInterface configureRowMeta( Mapping mapping, List<HBaseValueMeta> subsetOfCols ) {
    ...
    if ( subsetOfCols != null ) {
      ...
      ValueMetaInterface keyMeta = new ValueMeta( mapping.getKeyName(), kettleType );

      rowMeta.addValueMeta( keyMeta );

      // Add the rest of the fields in the mapping
      Map<String, HBaseValueMeta> mappedColumnsByAlias = mapping.getMappedColumns();
  ...
  public void testFullTableScanAllColsInMapping() throws Exception {
    ...
    HBaseInputData.initializeScan( conn, bu, tableMapping, null, null, null, null, null, vars );
  ...
  public void testFullTableScanSubsetOfColsInMapping() throws Exception {
    ...
    Map<String, HBaseValueMeta> colsMappedByAlias = tableMapping.getMappedColumns();
    ...
    RowMetaInterface outputRowMeta = configureRowMeta( tableMapping, subsetOfCols );
    conn.executeSourceTableScan();

    // for the key
    int expectedNumColsPerRow = RowDataUtil.OVER_ALLOCATE_SIZE + subsetOfCols.size();
    int count = 0;
    Object[] decodedRow = null;
    while ( conn.resultSetNextRow() ) {
      decodedRow =
          HBaseInputData.getOutputRow( conn, subsetOfCols, tableMapping.getMappedColumns(), tableMapping,
              outputRowMeta, bu );
  ...
  public void testTableScanLowerAndUpperKeyBounds() throws Exception {
    ...
    HBaseInputData.initializeScan( conn, bu, tableMapping, null, "-10", "50", null, null, vars );
  ...
  public void testTableScanEqualLowerAndUpperKeyBounds() throws Exception {
    ...
    HBaseInputData.initializeScan( conn, bu, tableMapping, null, "50", "50", null, null, vars );
  ...
  public void testTableScanTupleMapping() throws Exception {
    ...
    Mapping tableMapping = ma.getMapping( "MarksTestTupleTable", "MarksTestTupleMapping" );
    assertTrue( tableMapping != null );
    ...
    while ( conn.resultSetNextRow() ) {
      decodedRows =
          HBaseInputData.getTupleOutputRows( conn, null, tableMapping.getMappedColumns(), tableMapping, tupleHandler,
              outputRowMeta, bu );

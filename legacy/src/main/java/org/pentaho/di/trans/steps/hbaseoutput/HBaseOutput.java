  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    if ( first ) {
      ...
      try {
        ...
        m_hbAdmin =
            HBaseOutputData.getHBaseConnection( environmentSubstitute( m_meta.getZookeeperHosts() ),
                environmentSubstitute( m_meta.getZookeeperPort() ), environmentSubstitute( m_meta.getCoreConfigURL() ),
      ...
      try {
        ...
        if ( m_hbAdmin.isTableDisabled( targetName ) || !m_hbAdmin.isTableAvailable( targetName ) ) {
          throw new KettleException( BaseMessages.getString( HBaseOutputMeta.PKG,
      ...
      if ( m_meta.getMapping() != null && Const.isEmpty( m_meta.getTargetMappingName() ) ) {
        ...
        try {
          ...
          m_tableMapping =
              m_mappingAdmin.getMapping( environmentSubstitute( m_meta.getTargetTableName() ),
      ...
      try {
        ...
        if ( !Const.isEmpty( m_meta.getWriteBufferSize() ) ) {
          ...
          tableProps.setProperty( HBaseConnection.HTABLE_WRITE_BUFFER_SIZE_KEY, "" + writeBuffer );
    ...
    try {
      ...
      if ( !HBaseOutputData.initializeNewPut( getInputRowMeta(), m_incomingKeyIndex, r, m_tableMapping, m_bytesUtil,
          m_hbAdmin, !m_meta.getDisableWriteToWAL() ) ) {
    ...
    HBaseOutputData.addColumnsToPut( getInputRowMeta(), r, m_incomingKeyIndex, m_columnsMappedByAlias, m_hbAdmin,
        m_bytesUtil );
    ...
    try {
      ...
      String errorDescriptions =
          BaseMessages
              .getString( HBaseOutputMeta.PKG, "HBaseOutput.Error.ProblemInsertingRowIntoHBase", e.getMessage() );
      if ( getStepMeta().isDoingErrorHandling() ) {

  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    if ( first ) {
      ...
      try {
        ...
        m_hbAdmin =
            HBaseInputData.getHBaseConnection( environmentSubstitute( m_meta.getZookeeperHosts() ),
      ...
      try {
        ...
        if ( m_hbAdmin.isTableDisabled( sourceName ) || !m_hbAdmin.isTableAvailable( sourceName ) ) {
          throw new KettleException( BaseMessages.getString( HBaseInputMeta.PKG,
        ...
        for ( HBaseValueMeta vm : m_userOutputColumns ) {
          if ( !vm.isKey() ) {
            if ( m_columnsMappedByAlias.get( vm.getAlias() ) == null ) {
              throw new KettleException( BaseMessages.getString( HBaseInputMeta.PKG,
                  "HBaseInput.Error.UnableToFindUserSelectedColumn", vm.getAlias(), m_tableMapping.getTableName()
                      + HBaseValueMeta.SEPARATOR + m_tableMapping.getMappingName() ) );
            }
          } else {
      ...
      if ( m_userOutputColumns != null && m_userOutputColumns.size() > 0 && !m_tableMapping.isTupleMapping() ) {
        ...
        HBaseInputData.setScanColumns( m_hbAdmin, m_userOutputColumns, m_tableMapping );
      }

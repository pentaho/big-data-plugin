...
public class HBaseInputData extends BaseStepData implements StepDataInterface {
   ...
   * @param bytesUtil
   *          the byte conversion utils to use
   * @param tableMapping
   *          the table mapping info to use
   * @param dateOrNumberConversionMaskForKey
   *          conversion pattern for data/numbers
   * @param keyStartS
   *          the key start value
   * @param keyStopS
   *          the key stop value
   * @param scannerCacheSize
   *          the size of the scanner cache
   * @param log
   *          the log
   * @param vars
   *          variables
  ...
  public static void initializeScan( HBaseConnection hbAdmin, HBaseBytesUtilShim bytesUtil, Mapping tableMapping,
    ...
    if ( !Const.isEmpty( keyStartS ) ) {
        ...
        if ( !Const.isEmpty( convM ) && convM.length() > 0 ) {
          ...
          if ( tableMapping.getKeyType() == Mapping.KeyType.DATE
            ...
            try {
              ...
              keyLowerBound = HBaseValueMeta.encodeKeyValue( d, tableMapping.getKeyType(), bytesUtil );
            ...
            try {
              ...
              keyLowerBound = HBaseValueMeta.encodeKeyValue( num, tableMapping.getKeyType(), bytesUtil );
            } catch ( ParseException e ) {
              throw new KettleException( BaseMessages.getString( HBaseInputMeta.PKG,
                  "HBaseInput.Error.UnableToParseLowerBoundKeyValue", keyStartS ), e );
            }
      ...
      if ( !Const.isEmpty( keyStopS ) ) {
        ...
        if ( tableMapping.getKeyType() == Mapping.KeyType.BINARY ) {
          // assume we have a hex encoded string
          keyUpperBound = HBaseValueMeta.encodeKeyValue( keyStopS, tableMapping.getKeyType(), bytesUtil );
        } else if ( tableMapping.getKeyType() != Mapping.KeyType.STRING ) {
          ...
          if ( !Const.isEmpty( convM ) && convM.length() > 0 ) {
            if ( tableMapping.getKeyType() == Mapping.KeyType.DATE
              ...
              try {
                ...
                keyUpperBound = HBaseValueMeta.encodeKeyValue( d, tableMapping.getKeyType(), bytesUtil );
              } catch ( ParseException e ) {
                throw new KettleException( BaseMessages.getString( HBaseInputMeta.PKG,
              ...
              try {
                ...
                keyUpperBound = HBaseValueMeta.encodeKeyValue( num, tableMapping.getKeyType(), bytesUtil );
              } catch ( ParseException e ) {
                throw new KettleException( BaseMessages.getString( HBaseInputMeta.PKG,
                    "HBaseInput.Error.UnableToParseUpperBoundKeyValue", keyStopS ), e );
              }
    ...
    if ( !Const.isEmpty( scannerCacheSize ) ) {
      ...
      if ( log != null ) {
        log.logBasic( BaseMessages
            .getString( HBaseInputMeta.PKG, "HBaseInput.Message.SettingScannerCaching", cacheSize ) );
      }
    }
    try {
   ...
   *           if a problem occurs
   */
  public static void setScanColumns( HBaseConnection hbAdmin, List<HBaseValueMeta> limitCols, Mapping tableMapping )
    throws KettleException {
   ...
   *           if a problem occurs
   */
  public static void setScanFilters( HBaseConnection hbAdmin, Collection<ColumnFilter> columnFilters,
      boolean matchAnyFilter, Map<String, HBaseValueMeta> columnsMappedByAlias, VariableSpace vars )
    throws KettleException {
   ...
   *           if a problem occurs
   */
  public static List<Object[]> getTupleOutputRows( HBaseConnection hbAdmin, List<HBaseValueMeta> userOutputColumns,
      Map<String, HBaseValueMeta> columnsMappedByAlias, Mapping tableMapping, HBaseRowToKettleTuple tupleHandler,
      RowMetaInterface outputRowMeta, HBaseBytesUtilShim bytesUtil ) throws KettleException {
   ...
   *           if a problem occurs
   */
  public static Object[] getOutputRow( HBaseConnection hbAdmin, List<HBaseValueMeta> userOutputColumns,
      Map<String, HBaseValueMeta> columnsMappedByAlias, Mapping tableMapping, RowMetaInterface outputRowMeta,
    ...
    if ( userOutputColumns != null && userOutputColumns.size() > 0 ) {
      for ( HBaseValueMeta currentCol : userOutputColumns ) {
        if ( currentCol.isKey() ) {
          ...
          Object decodedKey = HBaseValueMeta.decodeKeyValue( rawKey, tableMapping, bytesUtil );
          ...
          Object decodedVal = HBaseValueMeta.decodeColumnValue( ( kv == null ) ? null : kv, currentCol, bytesUtil );

...
import org.pentaho.di.ui.vfs.HadoopVfsConnection;
...
public class HadoopVfsFileChooserDialog extends CustomVfsUiPanel {
  ...
  public void connect() {
    ...
    try {
      ...
      processUnsupportedSchemeMessage( exc );
    }
  ...
  /**Processes the exception with the specific way to show and log the more suitable error message
   *
   * @param exception
   */
  private void processUnsupportedSchemeMessage( KettleFileException exception ) {
    ...
    if ( message.contains( unsupportedSchemeMessagePrefix ) ) {
      try {
        ...
        showMessageAndLog( BaseMessages.getString( PKG, "HadoopVfsFileChooserDialog.error" ), BaseMessages.getString(
            PKG, "HadoopVfsFileChooserDialog.Connection.schemeError", hadoopConfigName ), message );
      } catch ( ConfigurationException ce ) {
        showMessageAndLog( BaseMessages.getString( PKG, "HadoopVfsFileChooserDialog.error" ), BaseMessages.getString(
            PKG, "HadoopVfsFileChooserDialog.Connection.error" ), ce.getMessage() );
      }
    } else {
      showMessageAndLog( BaseMessages.getString( PKG, "HadoopVfsFileChooserDialog.error" ), BaseMessages.getString( PKG,
          "HadoopVfsFileChooserDialog.Connection.error" ), exception.getMessage() );
    }
    return;
  }

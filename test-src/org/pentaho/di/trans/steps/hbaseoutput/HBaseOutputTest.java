...
public class HBaseOutputTest {
  ...
  protected void checkTableContents( FakeHBaseConnection conn, Mapping tableMapping,
      Map<String, HBaseValueMeta> colsMappedByAlias, List<HBaseValueMeta> subsetOfCols, CommonHBaseBytesUtil bu,
    ...
    while ( conn.resultSetNextRow() ) {
      if ( count < reference.size() ) {
        ...
        Object[] decodedRow =
            HBaseInputData.getOutputRow( conn, subsetOfCols, colsMappedByAlias, tableMapping, rowMeta, bu );
        for ( int i = 0; i < 3; i++ ) {
  ...
  public void testWrite() throws Exception {
    ...
    HBaseValueMeta keyMeta =
        new HBaseValueMeta( "BogusFamily" + HBaseValueMeta.SEPARATOR + tableMapping.getKeyName()
            + HBaseValueMeta.SEPARATOR + tableMapping.getKeyName(), ValueMetaInterface.TYPE_INTEGER, -1, -1 );
    keyMeta.setKey( true );
    ...
    for ( int i = 0; i < 100; i++ ) {
      ...
      assertTrue( HBaseOutputData.initializeNewPut( inputRowMeta, 0, inRow, tableMapping, bu, conn, true ) );

      HBaseOutputData.addColumnsToPut( inputRowMeta, inRow, 0, colsMappedByAlias, conn, bu );
      conn.executeTargetTablePut();
    }

    conn.closeTargetTable();

    checkTableContents( conn, tableMapping, colsMappedByAlias, subsetOfCols, bu, reference, inputRowMeta );
  }
  ...
  public void testWriteNullRowKey() throws Exception {
    ...
    assertFalse( HBaseOutputData.initializeNewPut( inputRowMeta, 0, inRow, tableMapping, bu, conn, true ) );
  }
}

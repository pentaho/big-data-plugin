...
public class AmazonHiveJobExecutorDialog extends JobEntryDialog implements JobEntryDialogInterface {
  ...
  public AmazonHiveJobExecutorDialog( Shell parent, JobEntryInterface jobEntry, Repository rep, JobMeta jobMeta )
    ...
    bf.createBinding(
        "jobentry-hadoopjob-flow-id", "value", controller, AmazonHiveJobExecutorController.HADOOP_JOB_FLOW_ID ); //$NON-NLS-1$ //$NON-NLS-3$
    ...
    bf.createBinding(
        "master-instance-type", "selectedItem", controller, AmazonHiveJobExecutorController.MASTER_INSTANCE_TYPE ); //$NON-NLS-1$ //$NON-NLS-2$
    bf.createBinding(
        "slave-instance-type", "selectedItem", controller, AmazonHiveJobExecutorController.SLAVE_INSTANCE_TYPE ); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    XulMenuList<String> masterInstanceType =
        (XulMenuList<String>) container.getDocumentRoot().getElementById( "master-instance-type" ); //$NON-NLS-1$
    masterInstanceType.setSelectedItem( "" + controller.getMasterInstanceType() );

    @SuppressWarnings( "unchecked" )//$NON-NLS-1$
    XulMenuList<String> slaveInstanceType =
        (XulMenuList<String>) container.getDocumentRoot().getElementById( "slave-instance-type" ); //$NON-NLS-1$

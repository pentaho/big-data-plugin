...
public class TagAssociations extends Resource {
  private static final String BASE_URL = CatalogClient.API_ROOT + "/tagassociation";
  private static final String LIST = BASE_URL + "/list";

  public TagAssociations( CatalogClient catalogClient ) {
    super( catalogClient );
  }

  public void write( List<TagAssociation> tagAssociations ) {
    StringEntity stringEntity = null;
    try {
      ObjectMapper objectMapper = new ObjectMapper();
      String body = objectMapper.writeValueAsString( tagAssociations );
      stringEntity = new StringEntity( body );
    } catch ( UnsupportedEncodingException | JsonProcessingException ignored ) {
      //Ignore
    }

    write( LIST, stringEntity, TagAssociation.class );
  }
}

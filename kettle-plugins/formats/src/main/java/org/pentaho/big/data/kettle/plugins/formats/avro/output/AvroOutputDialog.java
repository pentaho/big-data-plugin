...
public class AvroOutputDialog extends BaseAvroStepDialog<AvroOutputMeta> implements StepDialogInterface {
  ...
  private void addFieldsTab( CTabFolder wTabFolder ) {
    ...
    ColumnInfo[]
        parameterColumns =
        new ColumnInfo[] { new ColumnInfo( BaseMessages.getString( PKG, "AvroOutputDialog.Fields.column.Path" ),
  ...
  private void addSchemaTab( CTabFolder wTabFolder ) {
    ...
    props.setLook( wlSchemaPath ); new FD( wlSchemaPath ).left( 0, 0 ).top( 0, 0 ).apply();
    ...
    wbSchemaBrowse = new Button( wSourceGroup, SWT.PUSH ); props.setLook( wbBrowse );
    wbSchemaBrowse.setText( getMsg( "System.Button.Browse" ) );
    wbSchemaBrowse.addListener( SWT.Selection, event -> browseForFileInputPathForSchema() ); int
        bOffset =
        ( wbSchemaBrowse.computeSize( SWT.DEFAULT, SWT.DEFAULT, false ).y - wSchemaPath
    ...
    //      http://jira.pentaho.com/browse/BACKLOG-18706 - Tech Debt - Remove commented Code in AvroOutputDialog when
  ...
  private void addOptionsTab( CTabFolder wTabFolder ) {
    ...
    new FD( wCompression ).left( 0, 0 ).top( lCompression, FIELD_LABEL_SEP ).width( FIELD_SMALL + VAR_EXTRA_WIDTH )
        .apply();

  }

  protected ComboVar createComboVar( Composite container, String[] options ) {
    ComboVar combo = new ComboVar( transMeta, container, SWT.LEFT | SWT.BORDER ); combo.setItems( options );
    combo.addModifyListener( lsMod );
    return combo;
  }

  protected String getComboVarValue( ComboVar combo ) {
    String text = combo.getText(); String data = (String) combo.getData( text ); return data != null ? data : text;
  }
  ...
  // ui -> meta
  @Override protected void getInfo( AvroOutputMeta meta, boolean preview ) {
    meta.setFilename( wPath.getText() ); meta.setDocValue( wDocValue.getText() );
    meta.setNamespace( wNameSpace.getText() ); meta.setRecordName( wRecordName.getText() );
    meta.setSchemaFilename( wSchemaPath.getText() ); meta.setCompressionType( wCompression.getText() );
  ...
  private void populateFieldsUI( AvroOutputMeta meta, TableView wOutputFields ) {
    populateFieldsUI( meta.getOutputFields(), wOutputFields, ( field, item ) -> {
      int i = 1; item.setText( i++, coalesce( field.getPath() ) ); item.setText( i++, coalesce( field.getName() ) );
      item.setText( i++, coalesce( field.getTypeDesc() ) ); item.setText( i++, coalesce( field.getIfNullValue() ) );
  ...
  private void populateFieldsUI( List<FormatInputField> fields, TableView wFields,
    ...
    int nrFields = fields.size(); for ( int i = 0; i < nrFields; i++ ) {
      TableItem item = null; if ( i < wFields.table.getItemCount() ) {
  ...
  protected void getFields() {
    try {
      ...
      if ( r != null ) {
        ...
        BaseStepDialog.getFieldsFromPrevious( r, wOutputFields, 1, new int[] { 1 }, new int[] { 3 }, -1, -1, listener );
      }
  ...
  private void browseForFileInputPathForSchema() {
    try {
      String path = transMeta.environmentSubstitute( wSchemaPath.getText() ); VfsFileChooserDialog fileChooserDialog;
      String fileName; if ( Utils.isEmpty( path ) ) {
        fileChooserDialog = getVfsFileChooserDialog( null, null ); fileName = SCHEMA_SCHEME_DEFAULT + "://";
      } else {
        FileObject initialFile = getInitialFile( wSchemaPath.getText() );
        FileObject rootFile = initialFile.getFileSystem().getRoot();
        fileChooserDialog = getVfsFileChooserDialog( rootFile, initialFile ); fileName = null;
      }
      ...
      FileObject
          ...
          fileChooserDialog
              .open( shell, null, SCHEMA_SCHEME_DEFAULT, true, fileName, FILES_FILTERS, fileFilterNames, true,
                  VfsFileChooserDialog.VFS_DIALOG_OPEN_FILE_OR_DIRECTORY, true, true ); if ( selectedFile != null ) {
  ...
  @Override protected Listener getPreview() {
    return null;
  }
}
